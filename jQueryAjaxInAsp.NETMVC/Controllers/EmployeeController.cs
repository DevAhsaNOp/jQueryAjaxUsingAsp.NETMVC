...
namespace jQueryAjaxInAsp.NETMVC.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        IEnumerable<AjaxEmployee> GetAllEmployee()
        {
            using (DBModel db = new DBModel())
            {
                return db.AjaxEmployees.ToList<AjaxEmployee>();
            }
        ...
        {
            AjaxEmployee emp = new AjaxEmployee();
            if (id != 0)
            {
                using (DBModel db = new DBModel())
                {
                    emp = db.AjaxEmployees.Where(x => x.EmployeeID == id).FirstOrDefault<AjaxEmployee>();
                }
            }
            return View(emp);
        }
        ...
        [HttpPost]
        public ActionResult AddOrEdit(AjaxEmployee emp)
        {
            try
            {
        ...
        public ActionResult Delete(int id)
        {
            try
            {
                using (DBModel db = new DBModel())
                {
                    AjaxEmployee emp = db.AjaxEmployees.Where(x => x.EmployeeID == id).FirstOrDefault<AjaxEmployee>();
                    db.AjaxEmployees.Remove(emp);
                    db.SaveChanges();
                }
